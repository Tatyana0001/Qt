...
void MainWindow::StartRace(void){
    if(ui->rb_qtConcur->isChecked()){
        auto concurRace1_ = [&]{return concurRace1->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());};
        auto concurRace2_ = [&]{return concurRace2->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());};
        auto result = QtConcurrent::run(concurRace1_).then(concurRace2_);
    }
    else{
